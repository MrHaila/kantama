...
function main() {
  ...
  try {
    ...
    const routeCount = db.prepare('SELECT COUNT(*) as count FROM routes').get() as {
      count: number;
    };
    ...
    // Check for deciles
    const decileCount = db.prepare('SELECT COUNT(*) as count FROM deciles').get() as {
      count: number;
    };
