...
function calculateDeciles(db: Database.Database, force: boolean = false) {
  ...
  if (existingDeciles.count > 0 && !force) {
    console.log(
      `Deciles already exist (${existingDeciles.count} rows). Use --force to recalculate.`
    );
    return;
  }
  ...
  for (let decileNumber = 1; decileNumber <= 10; decileNumber++) {
    ...
    console.log(
      `Decile ${decileNumber}: ${label} (${routes.length > 0 ? routes[startIndex].duration : 0}s - ${maxDuration || '∞'}s)`
    );
