...
describe('clipZoneWithWater', () => {
  ...
  it('should handle zones entirely on land', () => {
    ...
    expect(turf.area(turf.feature(result as Polygon))).toBeCloseTo(turf.area(turf.feature(landZone)));
  });
  ...
  it('should handle MultiPolygon geometries', () => {
    const multiZone: MultiPolygon = {
      type: 'MultiPolygon',
      coordinates: [
        [[[1, 1], [4, 1], [4, 4], [1, 4], [1, 1]]], // Land island
        [[[6, 1], [9, 1], [9, 4], [6, 4], [6, 1]]]  // Water island
      ]
    };
