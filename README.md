...
An interactive map of temporal distances between places in the greater capital region (Helsinki, Espoo, Vantaa, Kauniainen) area using public transit.
...
- Transit data: HSL publishes their routing data (https://www.hsl.fi/en/hsl/open-data) and is involved in the OpenTripPlanner project (https://www.opentripplanner.org/) to consume the data. I ran this locally to avoid bombarding the public API hosted by (Digitransit)[https://digitransit.fi/en/].
...
1. Run `pnpm dev geocode` in the `varikko/` directory to find good reference points for each zone.
...
Opas will load data generated by Varikko to actually show the thing.
